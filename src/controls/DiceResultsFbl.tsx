...
import { SuccessDiceIcon } from "../icons/SuccessDiceIcon";
import { FailDiceIcon } from "../icons/FailDiceIcon";
import { VerticalAlignCenter } from "@mui/icons-material";

export function DiceResultsFbl({
    diceRoll,
    rollValues,
    expanded,
    onExpand,
    ...
    const finalValueSucess = useMemo(() => {
      return getSuccessDiceValueFbl(diceRoll, rollValues);
    }, [diceRoll, rollValues]);
    const finalValueFail = useMemo(() => {
        return getFailDiceValueFbl(diceRoll, rollValues);
      }, [diceRoll, rollValues]);
  
    return (
      <Stack alignItems="center" maxHeight="calc(100vh - 100px)" width="120%">
        <Tooltip
        ...
        >
          ...
          >
            <Typography variant="h4" color="white">
              {finalValueSucess} <SuccessDiceIcon   style={{verticalAlign:'middle'}} /> {finalValueFail} <FailDiceIcon style={{verticalAlign:"middle"}}/>
            </Typography>
          </Button>
        </Tooltip>
        <Grow
